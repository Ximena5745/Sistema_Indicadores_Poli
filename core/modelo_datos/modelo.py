...
class ModeloSGIND:
    def __init__(self, df_indicadores: pd.DataFrame, df_lineas: Optional[pd.DataFrame]=None,
                 df_objetivos: Optional[pd.DataFrame]=None, df_metas: Optional[pd.DataFrame]=None):
    ...
    def cumplimiento_por_linea(self, resultados: pd.DataFrame) -> pd.DataFrame:
        ...
        join = cierre.merge(self.df_lineas, on="ID", how="left") if self.df_lineas is not None else cierre
