...
def get_cmi_procesos_ids(year: Optional[int] = None, omit_if_no_cross: bool = True) -> set[str]:
    """
    ...
    - Si se provee `year`, se realiza un cruce con los IDs reportados
      por Kawak para ese año (carpeta `data/raw/Fuentes Consolidadas/`).
...
def filter_df_for_cmi_procesos(
    ...
    # Si no se pasa `year`, intentar inferir del DataFrame (columnas 'Anio','Año','Year')
    year_to_use = year
    if year_to_use is None:
        for yc in ("Anio", "Año", "Year", "year"):
            if yc in df.columns:
                try:
                    year_to_use = int(pd.to_numeric(df[yc], errors="coerce").dropna().max())
                except Exception:
                    year_to_use = None
                break

    valid_ids = get_cmi_procesos_ids(year=year_to_use, omit_if_no_cross=omit_if_no_cross)
    if not valid_ids:
        if omit_if_no_cross:
