...
class TestNormalizarYCategorizarIntegracion:
    ...
    def test_aplicar_en_series(self):
        ...
        df = pd.DataFrame(
            {"Cumplimiento_pct": [95, 100, 105, 75], "Id": [1, 9999, 9999, 1]}  # IDs válidos
        )

        df["Categoria"] = df.apply(
            lambda row: normalizar_y_categorizar(
                row["Cumplimiento_pct"], es_porcentaje=True, id_indicador=str(row["Id"])
            ),
            axis=1,
        )
...
class TestEdgeCases:
    ...
    def test_muy_proximo_a_umbral_pa(self):
        ...
        assert (
            normalizar_y_categorizar(95.00, es_porcentaje=True, id_indicador="1") == "Cumplimiento"
        )
        assert (
            normalizar_y_categorizar(95.01, es_porcentaje=True, id_indicador="1") == "Cumplimiento"
        )
