...
def _with_ui_group_key(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return df

    out = df.copy()
    if "Id" in out.columns:
        out["_ui_group_key"] = out["Id"].apply(_normalize_id)
    elif "Indicador" in out.columns:
        out["_ui_group_key"] = out["Indicador"].astype(str).str.strip().str.lower()
    else:
        out["_ui_group_key"] = ""
    out["_ui_group_key"] = out["_ui_group_key"].astype(str).str.strip()
    return out
...
def _render_subtab_objetivos(df_linea, linea, pdi_catalog=None):
    ...
    for obj in objetivos:
        with st.expander(f"Objetivo estratégico: {obj}"):
            ...
            if metas:
                st.caption("Agrupación UI activa: cada indicador se muestra una sola vez por objetivo para evitar duplicados visuales.")
                for meta in metas:
                    with st.expander(f"Meta Estratégica: {meta}"):
                        ...
                        df_meta = _with_ui_group_key(df_meta)
                        if not df_meta.empty:
                            n_before = len(df_meta)
                            df_meta = df_meta[df_meta["_ui_group_key"] != ""]
                            df_meta = df_meta.drop_duplicates(subset=["_ui_group_key"], keep="first")
                            if ids_rendered_obj:
                                df_meta = df_meta[~df_meta["_ui_group_key"].isin(ids_rendered_obj)]

                            keys_new = set(df_meta["_ui_group_key"].tolist())
                            ids_rendered_obj.update(keys_new)
                            df_meta = df_meta.drop(columns=["_ui_group_key"], errors="ignore")

                            n_after = len(df_meta)
                            if n_before > n_after:
                                st.caption("Se ocultaron filas duplicadas en la vista para mantener una lectura única por indicador.")

                        _render_tabla_indicadores(df_meta)
            else:
                # Si no hay metas estratégicas, mostrar todos los indicadores del objetivo
                df_obj_ui = _with_ui_group_key(df_obj)
                if not df_obj_ui.empty:
                    df_obj_ui = df_obj_ui[df_obj_ui["_ui_group_key"] != ""]
                    df_obj_ui = df_obj_ui.drop_duplicates(subset=["_ui_group_key"], keep="first")
                    df_obj_ui = df_obj_ui.drop(columns=["_ui_group_key"], errors="ignore")
                _render_tabla_indicadores(df_obj_ui)
