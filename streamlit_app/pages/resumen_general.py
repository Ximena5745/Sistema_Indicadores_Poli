...
def render():
    ...
    # --- Cargar datos de proyectos (para cumplimiento histórico) ---
    def _cargar_datos_proyectos():
        ...
        base = load_worksheet_flags()
        if base.empty or "Proyecto" not in base.columns or "Id" not in base.columns:
            return pd.DataFrame()
        
        ids_proyectos = set()
        for val in base.loc[base["Proyecto"] == 1, "Id"].dropna():
            if isinstance(val, float):
                ids_proyectos.add(str(int(val)))
            else:
                ids_proyectos.add(str(val).strip())
        
        if not ids_proyectos:
            return pd.DataFrame()
        
        cierres_proy = cierres[cierres["Id"].astype(str).isin(ids_proyectos)].copy()
        
        if cierres_proy.empty:
            return pd.DataFrame()
        
        cierres_proy = cierres_proy[cierres_proy["Anio"].notna() & (cierres_proy["Anio"] < 2026)].copy()
    ...
    proyectos_df = _cargar_datos_proyectos()
    ...
    # --- Resumen de Proyectos Institucionales ---
    st.markdown("### Proyectos Institucionales")
    
    if proyectos_df.empty or "cumplimiento_pct" not in proyectos_df.columns:
        st.info("No hay datos de proyectos institucionales con cierre para mostrar. Los proyectos identificados en 'Indicadores por CMI' no tienen registros de cumplimiento en 'Consolidado Cierres'.")
    else:
        # Promedio de cumplimiento por año
        yearly_avg = (
            proyectos_df[proyectos_df["Anio"].notna()]
            .groupby("Anio")["cumplimiento_pct"]
            .mean()
            .reset_index()
        )
        yearly_avg = yearly_avg.sort_values("Anio")
        
        if not yearly_avg.empty:
            fig_yearly = go.Figure()
            fig_yearly.add_trace(go.Bar(
                x=yearly_avg["Anio"].astype(int).astype(str),
                y=yearly_avg["cumplimiento_pct"],
                marker_color="#3B82F6",
                text=yearly_avg["cumplimiento_pct"].apply(lambda x: f"{x:.1f}%"),
                textposition="outside"
            ))
            fig_yearly.update_layout(
                title="Cumplimiento Promedio por Año (Proyectos Institucionales)",
                xaxis_title="Año",
        ...
        if "Linea" in proyectos_df.columns:
            linea_proy_summary = (
                proyectos_df.groupby("Linea", dropna=False)
                .agg(
                    N_Proyectos=("Id", "count"),
                    Cumpl_Promedio=("cumplimiento_pct", "mean")
                )
                .reset_index()
            )
            
            if not linea_proy_summary.empty:
                st.markdown("#### Resumen por Línea Estratégica")
                cols_linea = st.columns(3)
                for idx, row in linea_proy_summary.iterrows():
                    with cols_linea[idx % 3]:
                        cumplimiento = row["Cumpl_Promedio"] if pd.notna(row["Cumpl_Promedio"]) else 0
                        st.metric(
                            label=row["Linea"] if pd.notna(row["Linea"]) else "Sin Línea",
                            value=f"{cumplimiento:.1f}%",
                            delta=f"{int(row['N_Proyectos'])} proyectos"
                        )
                
                # Listado de proyectos por Línea
                st.markdown("#### Proyectos por Línea Estratégica")
                
                def _norm_key(s):
                    return str(s).lower().strip().replace(" ", "_").replace("ñ", "n").replace("á", "a").replace("é", "e").replace("í", "i").replace("ó", "o").replace("ú", "u")
                ...
                proyectos_df["Anio_int"] = proyectos_df["Anio"].astype(int)
                
                for def_linea in strategic_defs_proy:
                    proyectos_linea = proyectos_df[
                        proyectos_df["Linea"].apply(lambda x: def_linea["key"] in _norm_key(str(x)))
                    ].copy()
                    ...
                    display_cols = ["Indicador", "Anio_int", "cumplimiento_pct"]
                    display_df = proyectos_linea[display_cols].copy()
                    display_df = display_df.rename(columns={
                        "Indicador": "Proyecto",
                        "Anio_int": "Año Finalización",
                        "cumplimiento_pct": "% Avance"
                    })
                    display_df["% Avance"] = display_df["% Avance"].apply(lambda x: f"{x:.1f}%" if pd.notna(x) else "N/A")
