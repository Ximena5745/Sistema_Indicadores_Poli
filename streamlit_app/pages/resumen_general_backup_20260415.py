...
def render():
    ...
    lineas_disponibles = (
        sorted(
            ...
            else pdi_estrategico["Linea"].dropna().astype(str).unique().tolist()
        )
    ...
    if not pdi_estrategico.empty:
        ...
        if health_rate_e >= 70:
            insights_e.append(
                f"✅ El {health_rate_e}% de los indicadores PDI están en niveles saludables."
            )
        elif health_rate_e >= 50:
            insights_e.append(
                f"⚠️ El {health_rate_e}% de los indicadores PDI están en cumplimiento, con riesgo en algunos objetivos."
            )
        else:
            insights_e.append(
                f"🚨 Solo el {health_rate_e}% de los indicadores PDI cumplen expectativas; se requiere acción prioritaria."
            )
        ...
        if prev_month_e and best_improvements_e:
            insights_e.append(
                f"📈 Mejora destacada: \"{best_improvements_e[0]['name']}\" (+{best_improvements_e[0]['change']:.1f}%)."
            )

        if prev_month_e and worst_declines_e:
            insights_e.append(
                f"📉 Atención a \"{worst_declines_e[0]['name']}\" ({worst_declines_e[0]['change']:.1f}%)."
            )
    ...
    if (
        ...
        pdi_procesos = pdi_procesos.merge(
            map_df[["Subproceso", "Tipo de proceso"]].drop_duplicates(), on="Subproceso", how="left"
        )

        # Aplicar filtro de tipo si seleccionaron uno específico
        if tipo_proceso_seleccionado != "Todos":
            pdi_procesos = pdi_procesos[
                pdi_procesos["Tipo de proceso"] == tipo_proceso_seleccionado
            ]
        ...
        if not pdi_procesos.empty and "Tipo de proceso" in pdi_procesos.columns:
            ...
            if tipo_proceso_seleccionado == "Todos":
                tipos_unicos = sorted(
                    [
                        t
                        for t in pdi_procesos["Tipo de proceso"].dropna().unique()
                        if t in TIPOS_PROCESO
                    ]
                )
                ...
                if tipos_unicos:
                    ...
                    for idx, tipo in enumerate(tipos_unicos):
                        ...
                        n_subprocesos = (
                            df_tipo["Subproceso"].nunique()
                            if "Subproceso" in df_tipo.columns
                            else 0
                        )
                        cumpl_promedio = (
                            (df_tipo["cumplimiento_pct"].mean())
                            if "cumplimiento_pct" in df_tipo.columns
                            and not df_tipo["cumplimiento_pct"].isna().all()
                            else 0
                        )
                ...
                if subprocesos_del_tipo:
                    for i in range(0, len(subprocesos_del_tipo), 3):
                        ...
                        for idx, subproceso in enumerate(subprocesos_del_tipo[i : i + 3]):
                            ...
                            cumpl_prom = (
                                (df_subproceso["cumplimiento_pct"].mean())
                                if "cumplimiento_pct" in df_subproceso.columns
                                and not df_subproceso["cumplimiento_pct"].isna().all()
                                else 0
                            )
                    ...
                    st.info(
                        f"No hay subprocesos del tipo {tipo_proceso_seleccionado} con indicadores en este período."
                    )
            ...
            if prev_month_p:
                ...
                previous_process_data = previous_process_data[
                    previous_process_data["Año"] == prev_year_p
                ]

                if "Mes_num" in previous_process_data.columns:
                    previous_process_data = previous_process_data[
                        previous_process_data["Mes_num"] == prev_month_p
                    ]

                # Merge con tipos
                if (
                    not previous_process_data.empty
                    and "Subproceso" in previous_process_data.columns
                ):
                    ...
                    insights_p.append(
                        f"✅ El {health_pct_p}% de los indicadores por proceso están en niveles saludables."
                    )
